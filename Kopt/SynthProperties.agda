...
open import Kopt.Optimality using (kc-++ ; csc-++ ; rlen-++ ;
                                   over-𝒢-++ ; kfree-gp ; gp-circuit?)
...
decompose-ck-ok CK = ck-ok
decompose-ck-ok KC = kc-ok
...
-- and at most one CS gate. This is Kopt.GPData's exhaustive check
-- over the 6144 generalized permutations, which runs on their
-- permutation/phase data rather than on 4×4 matrices over 𝔻[i].

gperm-of-gp : (G : GP) ->
              Σ[ c ∈ Circuit ] ((gperm-of (gp-mat G) ≡ just c) × GPCircuit (gp-mat G) c)
gperm-of-gp G = go (gperm-of-gp-data G)
  where
    go : Σ[ c ∈ Circuit ] ((gperm-of (gp-mat G) ≡ just c) × GPProps (gp-mat G) c) ->
         Σ[ c ∈ Circuit ] ((gperm-of (gp-mat G) ≡ just c) × GPCircuit (gp-mat G) c)
    go (c , eq , sem , len , k , cs , ov) = c , eq , gp-props sem len k cs ov
